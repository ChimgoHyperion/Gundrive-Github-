...
public class firebulletprojectile : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            PhotonNetwork.Instantiate(HitEffect.name, transform.position, Quaternion.identity);
            if (collision.gameObject.GetComponent<MultiplayerMoveAndShoot>().photonView.ViewID != shooterId)
            {
                collision.gameObject.GetComponent<MultiplayerMoveAndShoot>().setshooter(shooterId);
            }
            if (collision.gameObject.GetComponent<MultiplayerMoveAndShoot>().photonView.ViewID == shooterId)
            {
                collision.gameObject.GetComponent<MultiplayerMoveAndShoot>().setshooter(0);
            }
